...
shinyServer(function(input, output) {
  ...
  output$scatterPlot <- renderPlot({
    ...
    plot <- ggplot() +
      ...
      scale_x_continuous() +
      scale_y_continuous() +
      labs(title='Residence Hall Crime Rates vs Males') +
...
#  df2 <- eventReactive(input$clicks2,{data.frame(fromJSON(getURL(URLencode('skipper.cs.utexas.edu:5001/rest/native/?query="select * from RESIDENCEHALLARREST2013"'),httpheader=c(DB='jdbc:oracle:thin:@sayonara.microlab.cs.utexas.edu:1521:orcl', USER='C##cs329e_pjo293', PASS='orcl_pjo293', MODE='native_mode', MODEL='model', returnDimensions = 'False', returnFor = 'JSON'), verbose = TRUE), ))})
#  KPI_Low_Max_value = 0.0018     
#  KPI_Medium_Max_value = 0.01
  
#  output$crosstab({renderPlot({
#    df2 <- df2 %>% group_by(INSTNM) %>% filter(LIQUOR12 != "null", DRUG12 != "null", WEAPON12 != "null", TOTAL != "null")
